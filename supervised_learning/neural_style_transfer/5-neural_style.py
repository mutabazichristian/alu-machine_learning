...
class NST:
    ...
    def generate_features(self):
        ...
        content_preprocessed = vgg19.preprocess_input(self.content_image * 255)
    ...
    def layer_style_cost(self, style_output, gram_target):
        ...
        if not isinstance(gram_target, (tf.Tensor, tf.Variable)) or len(gram_target.shape) != 3:
            raise TypeError("gram_target must be a tensor of shape [1, {c}, {c}] where {c} is the number of channels in style_output")

        # Calculate gram matrix of the style output
    ...
    def style_cost(self, style_outputs):
        ...
        # Calculate the weight for each layer (equal weights)
        weight_per_layer = 1.0 / len(self.style_layers)

        # Calculate the style cost for each layer and sum them up
        style_cost = 0
        for style_output, gram_target in zip(style_outputs, self.gram_style_features):
            ...
            style_cost += weight_per_layer * layer_cost
